...
// Everyone that joins gets the previous value instead of the next tick one 
// thats the difference between subject and BehaviorSubject
...
const subject$ = new Rx.BehaviorSubject('n/a');
temp$.subscribe(subject$);

meatspaceSystem((user) => {
  // TODO: notify users with `user.sendTemperature(temp)`
  const s = subject$.subscribe(temp => user.sendTemperature(temp))
